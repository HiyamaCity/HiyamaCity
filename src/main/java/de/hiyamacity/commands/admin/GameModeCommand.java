    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String s, @NotNull String[] args) {
        ...
        switch (args.length) {
            ...
            case 2:
                ...
                t.sendMessage(rs.getString("gmPrefix") + " " + trs.getString("gmOtherChangedOther").replace("%gamemode%", gm.name().toLowerCase().substring(0, 1).toUpperCase() + gm.name().toLowerCase().substring(1)).replace("%player%", p.getName()));
                t.setGameMode(gm);
                break;
        }
